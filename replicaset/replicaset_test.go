...
type MongoSuite struct {
	...
	root coretesting.MgoServer
}

func newServer(c *gc.C) coretesting.MgoServer {
	inst := coretesting.NewMgoServer("--replSet", rsName)
...
func dialAndTestInitiate(c *gc.C, inst coretesting.MgoServer, addr string) {
...
func (s *MongoIPV6Suite) TestAddRemoveSetIPv6(c *gc.C) {
	...
	// [::1]:port
	getAddr := func(inst coretesting.MgoServer) string {
...
func assertAddRemoveSet(c *gc.C, root coretesting.MgoServer, getAddr func(coretesting.MgoServer) string) {
	...
	instances := make([]coretesting.MgoServer, 5)
	instances[0] = root
