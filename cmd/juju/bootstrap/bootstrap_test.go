...
func (s *BootstrapSuite) TestBootstrapAPIReadyRetries(c *gc.C) {
	...
	for _, t := range []struct {
		...
		_, err := coretesting.RunCommand(c, NewCommand(), "-e", "devenv")
		if t.err == "" {
...
func (s *BootstrapSuite) run(c *gc.C, test bootstrapTest) testing.Restorer {
	...
	opc, errc := cmdtesting.RunCommand(cmdtesting.NullContext(c), NewCommand(), test.args...)
...
func (s *BootstrapSuite) TestRunEnvNameMissing(c *gc.C) {
	...
	_, err := coretesting.RunCommand(c, NewCommand())
...
func (s *BootstrapSuite) TestBootstrapTwice(c *gc.C) {
	...
	_, err := coretesting.RunCommand(c, NewCommand(), "-e", envName)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *BootstrapSuite) checkSeriesArg(c *gc.C, argVariant string) *cmd.Context {
	...
	ctx, err := coretesting.RunCommand(c, NewCommand(), "--upload-tools", argVariant, "foo,bar")
...
func (s *BootstrapSuite) TestBootstrapCleansUpIfEnvironPrepFails(c *gc.C) {
	...
	_, errc := cmdtesting.RunCommand(ctx, NewCommand(), "-e", "peckham")
...
func (s *BootstrapSuite) TestBootstrapJenvWarning(c *gc.C) {
	...
	_, errc := cmdtesting.RunCommand(ctx, NewCommand(), "-e", envName)
...
func (s *BootstrapSuite) TestInvalidLocalSource(c *gc.C) {
	...
	_, err := coretesting.RunCommand(c, NewCommand(), "--metadata-source", c.MkDir())
...
func (s *BootstrapSuite) TestAutoSyncLocalSource(c *gc.C) {
	...
	_, err := coretesting.RunCommand(c, NewCommand(), "--metadata-source", sourceDir)
	c.Assert(err, jc.ErrorIsNil)
...
func (s *BootstrapSuite) TestAutoUploadAfterFailedSync(c *gc.C) {
	...
	opc, errc := cmdtesting.RunCommand(cmdtesting.NullContext(c), NewCommand(), "-e", "devenv")
	c.Assert(<-errc, gc.IsNil)
...
func (s *BootstrapSuite) TestAutoUploadOnlyForDev(c *gc.C) {
	...
	_, errc := cmdtesting.RunCommand(cmdtesting.NullContext(c), NewCommand())
	err := <-errc
...
func (s *BootstrapSuite) TestMissingToolsUploadFailedError(c *gc.C) {
	...
	ctx, err := coretesting.RunCommand(c, NewCommand(), "-e", "devenv")
...
func (s *BootstrapSuite) TestBootstrapDestroy(c *gc.C) {
	...
	opc, errc := cmdtesting.RunCommand(cmdtesting.NullContext(c), NewCommand(), "-e", "brokenenv")
	err := <-errc
...
func (s *BootstrapSuite) TestBootstrapKeepBroken(c *gc.C) {
	...
	opc, errc := cmdtesting.RunCommand(cmdtesting.NullContext(c), NewCommand(), "-e", "brokenenv", "--keep-broken")
	err := <-errc
	c.Assert(err, gc.ErrorMatches, "failed to bootstrap environment: dummy.Bootstrap is broken")
	done := false
	for !done {
