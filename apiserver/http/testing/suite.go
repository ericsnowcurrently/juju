...
var binaryContentTypes = map[string]string{
	"":    apihttp.CTYPE_BINARY,
	"zip": "application/zip",
	"tgz": "application/x-tar-gz",
}

// BaseSuite is the base test suite for HTTP-related tests.
...
func (s *BaseSuite) SetUpTest(c *gc.C) {
	...
	s.Username = user.Tag().String()
}

func (s *BaseSuite) URL(c *gc.C, queryParts ...string) *url.URL {
	info := s.APIInfo(c)
	return &url.URL{
		Scheme:   "https",
		Host:     info.Addrs[0],
		Path:     "/environment/" + envUUID + s.Binding,
		RawQuery: strings.Join(queryParts, "&"),
	}
}

func (s *BaseSuite) SendRequest(c *gc.C, tag, password, method, ctype string, url *url.URL, body io.Reader) (*http.Response, error) {
...
func (s *BaseSuite) authRequest(c *gc.C, method, uri, contentType string, body io.Reader) (*http.Response, error) {
	return s.sendRequest(c, s.userTag, s.password, method, uri, contentType, body)
}

func (s *BaseSuite) uploadRequest(c *gc.C, uri, ctype, path string) (*http.Response, error) {
	contentType, ok := binaryContentTypes[ctype]
	if !ok {
		return nil, errors.Errorf("unknown binary type: %q", ctype)
	}
...
func (s *BaseSuite) assertErrorResponse(c *gc.C, resp *http.Response, expCode int, expError string) {
