...
// files.
func CreateBackup(
	dbinfo *DBConnInfo, stor BackupStorage, notes string, creator BackupCreator,
	...
	// Store the archive (and metadata).
	metadata := stor.NewMetadata(size, checksum, notes)
	_, err := stor.Add(metadata, archive)
	if err != nil {
		return nil, errors.Annotate(err, "error while storing backup archive")
	}
