...
type stateInfo struct {
    Config *config.Config  // environs.config.Config
    Username;
    Info *state.Info
}

func (si *stateInfo) set


func (sa *StateArchive) StateInfo() (stateInfo, error) {
    // Pull the archived config.
	agentConf, err := extractConfig(c.backupFile)
	if err != nil {
		return fmt.Errorf("cannot extract configuration from backup file: %v", err)
	}

    // Pull the local config.
...
func (c *Client) Restore(filename string) error {
	/*
