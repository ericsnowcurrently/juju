...
func (s *toolsSuite) TestToolsRequiresPOST(c *gc.C) {
	resp := s.authRequest(c, "PUT", s.toolsURI(c, ""), "", nil)
...
func (s *toolsSuite) TestToolsAuthRequiresUser(c *gc.C) {
	...
	resp = s.authRequest(c, "POST", s.toolsURI(c, ""), "", nil)
...
func (s *toolsSuite) TestToolsUploadFailsWithNoTools(c *gc.C) {
	...
	resp := s.uploadRequest(c, s.toolsURI(c, "?binaryVersion=1.18.0-quantal-amd64"), true, tempFile.Name())
...
func (s *toolsSuite) TestToolsUploadFailsWithInvalidContentType(c *gc.C) {
	...
	resp := s.uploadRequest(c, s.toolsURI(c, "?binaryVersion=1.18.0-quantal-amd64"), false, tempFile.Name())
...
func (s *toolsSuite) TestToolsUpload(c *gc.C) {
...
func (s *toolsSuite) TestToolsUploadAllowsTopLevelPath(c *gc.C) {
	...
	url.Path = "/tools"
	resp := s.uploadRequest(c, url.String(), true, toolPath)
	// Check the response.
	stor := s.Environ.Storage()
	toolsURL, err := stor.URL(tools.StorageName(vers))
	c.Assert(err, gc.IsNil)
...
func (s *toolsSuite) TestToolsUploadFakeSeries(c *gc.C) {
	...
	resp := s.uploadRequest(c, s.toolsURI(c, params), true, toolPath)
