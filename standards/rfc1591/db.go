...
type Database struct {
	tlds  map[string]TLD
	types map[TLDType]bool
}

func newDatabase(types []TLDType, tlds []TLD) (Database, error) {
	db := Database{
		tlds:  make(map[string]TLD, len(tlds)),
		types: make(map[TLDType]bool, len(types)),
	}
	for i, typ := range types {
		if err := typ.Validate(); err != nil {
			return Database{}, fmt.Errorf("bad TLD type %d/%d (%s): %v", i, len(types), typ, err)
		}
		db.types[typ] = true
	}
	for i, tld := range tlds {
		if err := db.addTLD(tld); err != nil {
			return Database{}, fmt.Errorf("bad TLD %d/%d (%s): %v", i, len(tlds), tld.Name, err)
		}
	}
	return db, nil
}

func (db Database) LookUp(name rfc1035.Label) (TLD, bool) {
	if name.IsRoot() {
		return TLD{}, true
	}
	tld, ok := db.tlds[name.String()]
	return tld, ok
}

type TLDer interface {
	TLD() rfc1035.Label
}

func (db Database) LookUpDomain(domain TLDer) (TLD, bool) {
	name := domain.TLD()
	return db.LookUp(name)
}

func (db Database) LookUpString(name string) (TLD, error) {
	domain, err := rfc5890.ParseDomainName(name)
	if err != nil {
		return TLD{}, fmt.Errorf("could not parse string: %v", err)
	}
	tld, ok := db.LookUpDomain(domain)
	if !ok {
		return tld, &errTLDNotFound{domain.TLD().String()}
	}
	return tld, nil
}

type errTLDNotFound struct {
	name string
}

func (err errTLDNotFound) Error() string {
	return fmt.Sprintf("TLD %q not in database", err.name)
}

func IsErrTLDNotFound(err error) bool {
	switch err.(type) {
	case *errTLDNotFound, errTLDNotFound:
		return true
	default:
		return false
	}
...
func (db Database) AsMap() map[string]TLD {
	tlds := make(map[string]TLD, len(db.tlds))
	for _, tld := range db.tlds {
		tlds[tld.Name.String()] = tld
	}
	return tlds
}

func (db Database) addTLD(tld TLD) error {
	// tld is a copy.
	if tld.IsRoot() {
		return fmt.Errorf("root TLD not supported")
	}
	if err := db.validateTLD(tld); err != nil {
		return fmt.Errorf("invalid: %v", err)
	}
	name := tld.Name.String()
	if _, ok := db.tlds[name]; ok {
...
func (db Database) IsSupportedTLDType(typ TLDType) bool {
	return db.types[typ]
}

func (db Database) validateTLD(tld TLD) error {
	if err := tld.Validate(); err != nil {
		return err
	}
	if !db.IsSupportedTLDType(tld.Type) {
		return fmt.Errorf("unsupported TLD type %#v", tld.Type)
	}
	return nil
}

// DB holds the IANA database of top level domains.
var DB = func(rows [][3]string) Database {
	tlds := make([]TLD, len(rows))
	for i, row := range rows {
		name, err := rfc5890.NewLabel(row[0])
		if err != nil {
			panic(fmt.Sprintf("bad name in IANA database: %v", err))
		}
		tlds[i] = TLD{
			Name: name,
			Type: TLDType(row[1]),
	...
	db, err := newDatabase(types, tlds)
	if err != nil {
		panic(fmt.Sprintf("bad IANA database: %v", err))
	}
	return db
}(db)
...
var types = []TLDType{
	TLDTypeCountryCode,
	TLDTypeGeneric,
	TLDTypeGenericRestricted,
	TLDTypeInfrastructure,
	TLDTypeSponsored,
	TLDTypeTest,
}
